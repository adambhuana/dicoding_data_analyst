...
min_date = df1["order_approved_at"].min()
max_date = df1["order_approved_at"].max()

# Sidebar
with st.sidebar:
    col1, col2 = st.columns(2)
    with col1:
        url = "https://github.com/adambhuana/dicoding_data_analyst/"
        st.write("[Profil Adam Puspabhuana](%s)" % url)
    with col2:
        st.write(' ')
...
# Main
main_df = df1[(df1["order_approved_at"] >= str(start_date)) & 
                 (df1["order_approved_at"] <= str(end_date))]
...
st.caption('Dikerjakan oleh Adam Puspabhuana sebagai syarat kelulusan program Dicoding')
